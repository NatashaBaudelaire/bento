...
class Profile(commands.Cog):
    ...
    async def profile(self, ctx, member: discord.Member = None):
        ...
        if not data:
            return await ctx.send("⚠️ Could not load profile data.")
        ...
        level, xp_in_level, xp_needed = db.get_level_progress(xp)

        progress_blocks = int((xp_in_level / xp_needed) * 20) if xp_needed > 0 else 20
        progress_blocks = max(0, min(20, progress_blocks))
        bar = "🟦" * progress_blocks + "⬜" * (20 - progress_blocks)

        accuracy = (correct / total * 100) if total > 0 else 0

        study_pref = await db.get_study_preference(user_id)
        if study_pref:
            study_text = f"**{study_pref['subject']}** — *{study_pref['content']}*"
        else:
        ...
        embed.add_field(
            ...
            value=f"**{daily}/10** correct answers today",
            inline=False,
        )

        embed.add_field(name="📖 Current Focus", value=study_text, inline=False)
